...
client = OpenAI(api_key='ENTER-YOUR-API-KEY')  # Set your OpenAI API key here
...
class GPT4ConversationNode(Node):
    ...
    def get_gpt4_response(self, query):
        try:
            ...
            prompt = "you are a professional robot operator that can move your robot in four ways: move, turn_left, turn_right, and stop. The robot can also bark. Given a user's command, output a list of instructions that best represent the request."
