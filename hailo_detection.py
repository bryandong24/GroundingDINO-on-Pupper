...
from std_msgs.msg import Float32MultiArray # Bryan: For creating a new publisher for the reddest object position
...
class HailoDetectionNode(Node):
    def __init__(self):
        ...
        self.reddest_pub = self.create_publisher(Float32MultiArray, 'reddest_object_position', 10) # Bryan: For publishing the reddest object position
    ...
    # Compute the redness of the object in the bounding box 
    def compute_redness(self, frame, left_x, right_x, top_y, bottom_y) -> float:
        left_x = int(left_x)
        right_x = int(right_x)
        top_y = int(top_y)
        bottom_y = int(bottom_y)
        red_channel = frame[top_y:bottom_y, left_x:right_x, 0]
        green_channel = frame[top_y:bottom_y, left_x:right_x, 1]  
        blue_channel = frame[top_y:bottom_y, left_x:right_x, 2]
        # Compute mean of red channel
        average_redness = np.mean(red_channel)
        average_g = np.mean(green_channel)
        average_b = np.mean(blue_channel)
        print("rgb?", red_channel, green_channel, blue_channel)
        return average_b / (average_redness + average_g + average_b)  # WE CHANGED TO BLUE
...
################# TESTING TO SEE IF AVERAGE REDNESS COMPUTATION WORKS ########
        # Calculate average redness
        # Get the red channel (now index 0 after the ::-1 channel swap)
        red_channel = frame[:, :, 0]
        # Compute mean of red channel
        average_redness = np.mean(red_channel)
...
###################### END OF TESTING ########################################
        ...
        for i in range(detections["num_detections"]):
            ...
            left_x = det.bbox.center.position.x - det.bbox.size_x/2
            right_x = det.bbox.center.position.x + det.bbox.size_x/2
            top_y = det.bbox.center.position.y - det.bbox.size_y/2
            bottom_y = det.bbox.center.position.y + det.bbox.size_y/2

            # Bryan: Compute redness of the detection
            redness = self.compute_redness(frame, left_x, right_x, top_y, bottom_y)
            print(f"Redness of detection {i}: {redness}")
            redness_data.append(redness)
        ...
        redness_msg = Float32MultiArray(data = redness_data)
        #redness.data = redness_data
        self.reddest_pub.publish(redness_msg) # Bryan: Publish the redness of the object
